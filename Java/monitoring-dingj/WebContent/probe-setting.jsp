<%@page import="monitoring.manager.MQClient"%>
...
<%
	...
	request.setAttribute("probes", DBHelper.INSTANCE.queryProbeByMachineName(mName));
	request.setAttribute("types", MQClient.getInstance().getTypeList());
%>
<!DOCTYPE html>
<html>
<head>
...
<script type="text/javascript">
$(function() {
	$("#set-probe").click(function() {
		var type = $("#type-selector").val();
		var period = parseInt($("#period-input").val(),10);
		if (isNaN(period) || period <=0) return;
		DB.insertProbe(type, "${requestScope.name}", period, function() {
			location.reload();
		});
	});
});
...
<body>
	...
	<div class="row" style="padding-top: 50px;">
		...
		<ul class="nav nav-tabs">
		  <li class="active"><a href="#">Probe Settings</a></li>
		  <li><a href="event-viewer.jsp?name=${requestScope.name }">Event Viewer</a></li>
		...
		<fieldset>
			...
			<select id="type-selector" class="form-control">
				<c:forEach var="p" items="${requestScope.types}">
					<option value="${p }">${p }</option>
				</c:forEach>
			</select>
			<label for="period-input">Period: (seconds)</label>
			<input class="form-control" type="text" id="period-input" class="form-control"/>
			<button type="button" class="btn btn-success" id="set-probe">Set Probe</button>
			</fieldset>
		</div>
		<div class="col-md-8">
		...
		<thead><tr><th>#</th><th>Type</th><th>Monitoring Period (seconds)</th></tr></thead>
		<tbody>
		<c:forEach var="p" items="${requestScope.probes }">
		<tr>
		<td>${p.id }</td>
		<td>${p.type }</td>
		<td>${p.period }</td>

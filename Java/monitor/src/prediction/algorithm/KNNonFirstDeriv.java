...
public class KNNonFirstDeriv implements PredictionAlgorithm {

	public int K = 5;
	public int patternLen = 50;

	public KNNonFirstDeriv meiose(){
		KNNonFirstDeriv cel = new KNNonFirstDeriv();
		double xray = Math.random();
		if(xray < 1/3){
			double proba = 0.5 - 0.5 / (1 + Math.exp(-0.3*K+4)); // sigmoid
			cel.K = (Math.random() < proba | K==1) ? K+1 : K-1;
		}else if(xray < 2/3){
	...
	public TimeSeries effectuerUnePrediction(TimeSeries ts, int startIndex, int nbOfStepsToBePredicted) {
		...
		TimeSeries deriv = deriver(ts);
		PatternStructKNN[] neighboors = euclidKNN(deriv, startIndex);
		ArrayList<Serie> seriesPred = new ArrayList<Serie>(); 
		for(Serie s : deriv.series){
		...
		return new TimeSeries("KNN prediction", decalage, deriv.timeStep, integrer(startIndex, seriesPred,ts));
	}

	@Override
	public boolean equals(Object obj) {
		if(obj.getClass() == KNNonFirstDeriv.class){
			KNNonFirstDeriv o2 = (KNNonFirstDeriv) obj;
	...
	public String toString() {
		return "KNN deriv \t K="+K+"\t patLen="+patternLen;
	}
	...
	private TimeSeries deriver(TimeSeries ts) {
		
		ArrayList<Serie> lag1 = new ArrayList<Serie>(); 
		for (Serie s : ts.series) {
			double[] val = new double[s.value.length-1];
			for (int i = 0 ; i < s.value.length-1 ; i++) {
				val[i] = s.value[i+1] - s.value[i];
			}
			lag1.add(new Serie(s.name, val));
		}
		return new TimeSeries("lag1", ts.timeStart, ts.timeStep, lag1);
	}
	
	private ArrayList<Serie> integrer(int startIndex, ArrayList<Serie> seriesPred, TimeSeries ts) {
		for(Serie s : seriesPred) {
			double baseValue = ts.getSerie(s.name).value[startIndex];
